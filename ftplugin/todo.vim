...
function! s:todo_fold_level(lnum)
    ""
    " The match function returns the index of the matching pattern
    "   or -1 if the pattern doesn't match.
    " In this case, we always try to match a completed task from the beginning
    "   of the line so that the matching function will always return -1
    "   if the pattern doesn't match or 0 if the pattern matches.
    " Incrementing by one the value returned by the matching function,
    "   we will return 1 for the completed tasks
    "   (they will be at the first folding level) while for the other lines
    "   0 will be returned, indicating that they do not fold.
